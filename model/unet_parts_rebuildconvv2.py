...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None, softmax=False):
        super().__init__()
        if not mid_channels:
            mid_channels = out_channels
        second_conv = WeightConv2d if softmax else nn.Conv2d 
        self.double_conv = nn.Sequential(
            nn.Conv2d(in_channels, mid_channels, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),
            second_conv(mid_channels, out_channels, kernel_size=3, padding=1, bias=False),
...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConv(in_channels, out_channels, softmax=True)
        )
