...
class BasicMetric(nn.Module):
    ...
    def process(self, inputs, target, softmax=False, one_hot=True):
        """
        ...
        if one_hot:
            target = self._one_hot_encoder(target, inputs.shape).float()
...
class DiceLoss(BasicMetric):
    ...
    def forward(self, inputs, target, weight=None, softmax=False, one_hot=True):
        """
        input: batch, classes, ... float
        target: batch, ... int
        """
        inputs, target = self.process(inputs, target, softmax, one_hot)
        if weight is None:
